...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double prob = 1./(1+exp(-2*_beta*(_J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin())+_H))); //Probability of spin 1
    double rand = _rnd.Rannyu();
    if(rand < prob){
      _particle(i).setspin(1); //Set spin to 1
    }
    else{
      _particle(i).setspin(-1); //Set spin to -1
    }
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    } else if( property == "SALTA_NBLOCKS" ){
      input >> _nblocks_salto;
      coutf << "SALTA_NBLOCKS= " << _nblocks_salto << endl;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        _measure_gofr = true;
        _index_gofr.resize(_n_bins);
        for(int i = 0; i < _n_bins; i++){
          _index_gofr[i] = i + index_property;
        }
    ...
    if(coutf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        coutf << "LJ" << "  " 
              ...
              << setw(16) << _particle(i).getposition(2,false)/_side(2) << endl; // z
      }
...
void System :: write_XYZ(int nconf){
  ...
  if(coutf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      coutf << "LJ" << "  " 
            << setw(16) << _particle(i).getposition(0,false)          // x
            << setw(16) << _particle(i).getposition(1,false)          // y
            << setw(16) << _particle(i).getposition(2,false) << endl; // z
    }
  } else cerr << "PROBLEM: Unable to open config.xyz" << endl;
  coutf.close();
  return;
...
void System :: read_configuration(){
  ifstream cinf;
  string nameopenfile = "xyz";
  if(_sim_type > 1) nameopenfile = "ising";
  cinf.open("../INPUT/CONFIG/config." + nameopenfile);
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & config." << nameopenfile << " not match!" << endl;
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr){  // if I want to measure gofr
          if(dr < _halfside.min()){  // upper cut on histogramm
            _measurement(_index_gofr[int(dr/_bin_size)]) += 2; // increment by 2 if particle at distance r, r+dr
          }
        }
  ...
  if (_measure_pressure){
    pressure_temp = _ptail + _measurement(_index_temp)*_rho + 48./(3*_volume) * pressure_temp;
...
// TO BE FIXED IN EXERCISE 6
  if(_measure_chi){
    ...
    for (int i=0; i<_npart; i++){
      ...
      _measurement(_index_chi) = susc*susc*_beta/double(_npart);
  }
...
void System :: averages(int blk){
  ...
  if(_measure_gofr && blk == _nblocks){  // print only at the end of the simulation
    coutf.open("../OUTPUT/gofr.dat",ios::app);
    for(int i = 0; i < _n_bins; i++){
      average = _average(_index_gofr[i])/(_rho*_npart*4./3.*M_PI*(pow((i+1)*_bin_size,3)-pow(i*_bin_size,3)));  // useless to print
      sum_average = _global_av(_index_gofr[i])/(_rho*_npart*4./3.*M_PI*(pow((i+1)*_bin_size,3)-pow(i*_bin_size,3)));  // normalized average of all blocks
      sum_ave2 = _global_av2(_index_gofr[i])/pow(_rho*_npart*4./3.*M_PI*(pow((i+1)*_bin_size,3)-pow(i*_bin_size,3)),2);
      coutf << setw(20) << i*_bin_size
            << setw(20) << average
            << setw(20) << sum_average/(double(blk))
            << setw(20) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  if (_measure_cv){
    ...
    average = _npart*(_average(_index_cv)-_average(_index_tenergy)*_average(_index_tenergy))*_beta*_beta;
    _global_av_cv += average;
    _global_av2_cv += average*average;
    coutf << setw(20) << blk
          << setw(20) << average
          << setw(20) << _global_av_cv/double(blk)
          << setw(20) << this->error(_global_av_cv, _global_av2_cv, blk) << endl;
    coutf.close();
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
...
int System :: get_nblocks_salto(){
  return _nblocks_salto;
}
